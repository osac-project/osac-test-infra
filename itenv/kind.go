...
// AddSchemeFunc adds a function that will be called to register custom types with the Kubernetes client scheme.
// This replaces the hardcoded scheme registration and allows each consumer to register its own CRD types.
func (b *KindBuilder) AddSchemeFunc(fn func(*runtime.Scheme) error) *KindBuilder {
	b.schemeFuncs = append(b.schemeFuncs, fn)
	return b
}
...
func (b *KindBuilder) Build() (result *Kind, err error) {
	...
	result = &Kind{
		...
		schemeFuncs:  slices.Clone(b.schemeFuncs),
	}
	return
}
...
// Start creates or reuses the Kind cluster and sets up the kubeconfig and Kubernetes clients.
// It returns true if the cluster was already existing (reused), false if newly created.
// After Start, call the Install* methods to set up components as needed.
func (k *Kind) Start(ctx context.Context) (existed bool, err error) {
	existed, err = k.Exists(ctx)
	if err != nil {
		err = fmt.Errorf("failed to check if cluster exists: %w", err)
		return
	}
	if existed {
		k.logger.LogAttrs(ctx, slog.LevelDebug, "Using existing kind cluster")
	} else {
		k.logger.LogAttrs(ctx, slog.LevelDebug, "Creating new kind cluster")
		err = k.createCluster(ctx)
		if err != nil {
			err = fmt.Errorf("failed to create kind cluster '%s': %w", k.name, err)
			return
		}
		k.logger.LogAttrs(ctx, slog.LevelDebug, "Created new kind cluster")
	}
