...
test-bmaas:
	mkdir -p $(REPORTS_DIR)
	pytest tests/bmaas/ -n 0 -v --tb=short $(if $(MARKER),-m "$(MARKER)") $(if $(TEST),-k "$(TEST)") --junitxml=$(REPORTS_DIR)/bmaas.xml

test-bmaas-networking test-bmaas_networking:
	mkdir -p $(REPORTS_DIR)
	pytest tests/bmaas_networking/ -n 0 -v --tb=short $(if $(TEST),-k "$(TEST)") --junitxml=$(REPORTS_DIR)/bmaas-networking.xml
