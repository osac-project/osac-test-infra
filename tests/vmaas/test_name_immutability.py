...
class TestVirtualNetworkImmutability:
    ...
    def test_update_name_rejected(self, jwt_grpc_tenant1: GRPCClient, network_class: str) -> None:
        vn_name = f"imm-name-{uuid.uuid4().hex[:8]}"
        vn_id: str | None = None
        try:
            vn_id = jwt_grpc_tenant1.create_virtual_network(
                name=vn_name, network_class=network_class, ipv4_cidr="10.130.0.0/16"
            )
                ...
                jwt_grpc_tenant1.call(
                    ...
                    data={
                        "object": {
                            ...
                            "spec": {"network_class": {"name": network_class}, "ipv4_cidr": "10.130.0.0/16"},
                        },
    ...
    @pytest.mark.skip(reason="Cannot test tenant/project immutability - referencing non-existent tenant fails with PermissionDenied before immutability check")
    @pytest.mark.parametrize(
        ("field_path", "field_key", "field_value"),
        [
            ("metadata.tenant", "tenant", "different-tenant"),
            ("metadata.project", "project", "different-project"),
        ],
        ids=["tenant", "project"],
    )
    def test_update_tenant_or_project_rejected(
        self, jwt_grpc_tenant1: GRPCClient, network_class: str, field_path: str, field_key: str, field_value: str
    ) -> None:
        vn_name = f"imm-tp-{uuid.uuid4().hex[:8]}"
        vn_id: str | None = None
        try:
            vn_id = jwt_grpc_tenant1.create_virtual_network(
                name=vn_name, network_class=network_class, ipv4_cidr="10.131.0.0/16"
            )
                ...
                jwt_grpc_tenant1.call(
                    ...
                    data={
                        "object": {
                            ...
                            "spec": {"network_class": {"name": network_class}, "ipv4_cidr": "10.131.0.0/16"},
                        },
