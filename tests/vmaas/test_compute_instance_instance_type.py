...
def _build_create_ci_args(
    ...
    args += [
        ...
        "--boot-disk-size", "20",
        "--boot-disk-storage-tier", storage_tier,
    ...
    try:
        ...
        dep_output, dep_rc = run_unchecked(
            *_build_create_ci_args(
                cli, vm_template, default_subnet, active_instance_type, default_storage_tier,
...
def test_compute_instance_nonexistent_instance_type(
    ...
    output, rc = run_unchecked(
        *_build_create_ci_args(
            cli, vm_template, default_subnet, missing_it_name, default_storage_tier, ci_name=ci_name,
        ),
    )
    if rc == 0:
...
def test_compute_instance_obsolete_instance_type(
    ...
    output, rc = run_unchecked(
        *_build_create_ci_args(
            cli, vm_template, default_subnet, active_instance_type, default_storage_tier, ci_name=ci_name,
        ),
    )
    if rc == 0:
        uuid_match = re.search(r"'([^']+)'", output)
