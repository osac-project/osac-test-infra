...
3. `<token>` is the controller's static `netris_auth_token` setting (not a per-user login token) — fetch it the same way `netris.controller.general`'s `read` role does: `POST /api/auth` with `{"user": "{{ netris_login }}", "password": "{{ netris_password }}", "auth_scheme_id": 1}` to get a `connect.sid` cookie, then `GET /api/v2/general` with that cookie and read the `netris_auth_token` entry from the response's `data` array. This lab's `group_vars/all.yml` defaults `netris_login`/`netris_password` to the well-known Netris platform defaults (`netris`/`netris`) — fine for an isolated dev lab, but rotate them (and re-point these vars) in any deployment reachable beyond it.
...
**Fix:** Rather than weakening reverse-path filtering host-wide (`default`/`all`, which would also affect this host's real uplink or any other VPN), the fix is scoped to only the two interfaces that actually need it, applied where each is created:
- `roles/cloudsim/tasks/main.yml` — zeroes `net.ipv4.conf.br-mgmt.rp_filter` right after Pulumi brings up the VMs/bridges.
- `roles/connectivity/tasks/vpn.yml` — zeroes `net.ipv4.conf.tun0.rp_filter` right after the VPN client's `tun0` comes up.
- `roles/prerequisites/tasks/main.yml` — covers the re-run case: if `br-mgmt`/`tun0` already exist from a prior deploy (so the two tasks above won't re-create them), fixes their `rp_filter` explicitly, since a value already set at interface-creation time doesn't change retroactively.

If you hit this on a host predating the fix, re-run `make connectivity` (or manually zero `net.ipv4.conf.br-mgmt.rp_filter` and `net.ipv4.conf.tun0.rp_filter` and restart `netris-sg` on each softgate).
