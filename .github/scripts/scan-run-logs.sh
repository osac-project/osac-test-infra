...
#                       cancelled-before-start) is N/A, not SCAN_OK=false.
...
# True when the run never scheduled a job (fork/env action_required, or
# cancelled before GitHub created jobs). Those have no log archive: the
# logs API 404s or returns a zip unzip cannot open. Fail closed (return 1)
# if the jobs list cannot be fetched -- we cannot prove emptiness.
run_has_no_jobs() {
