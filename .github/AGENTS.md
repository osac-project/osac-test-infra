...
- `.github/actions/check-e2e-readiness/` — Allow: `lgtm`, bot-applied `e2e-ready`, or `coderabbitai[bot]` APPROVED on head (blocked while a human still has `CHANGES_REQUESTED`). Otherwise `ready=false` and wait; do not fail. Human APPROVED does not unlock
